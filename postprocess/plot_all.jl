...
approx = readdlm(joinpath(@__DIR__, "..", "run", "ssw_roll_1d", "ssw_roll_wave", "sol.txt"))
exact_brock = readdlm(joinpath(@__DIR__, "exact_brock.txt"), skipstart = 1)
for i in axes(exact_brock, 1)
    global x = exact_brock[i, 1]
    if x > 1.0
        exact_brock[i, 1] = x - 1.0
    end
end
h0 = 7.98e-3
ax.scatter(exact_brock[:, 1] * 1.3, exact_brock[:, 2] * h0, label = "Experimental", lw = 2,
           c = "k")
plot_sol(ax, approx, 1, ls = "--")
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results", "roll_wave.pdf")
fig
...
plot_sol(ax, approx, 1, ls = "--")
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results", "roll_wave2.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
for i in 1:5
    ...
    global filename = joinpath(@__DIR__, "postprocessed_results", "jin_xin_burgers_eps$(epsilon_string).pdf")
    fig.savefig(filename)
    run(`bash pdfbb $filename`)
end
...
plot_sol(ax, approx_500, 3, ls = "dashed", label = "\$M=500\$")
plot_sol(ax, approx_1000, 3, ls = "dotted", color = "blue", label = "\$M=1000\$")
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results/", "tmp_super_stiff_rho.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results/", "tmp_super_stiff_v1.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.legend()
fig
filename = joinpath(@__DIR__, "postprocessed_results", "burgers_non_linear.pdf")
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.legend()
fig
filename = joinpath(@__DIR__, "postprocessed_results", "burgers_rare_non_linear.pdf")
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.plot(domain, exact.(domain), label = "Exact", c = "k", lw = 2, ls = "-")
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results", "sup_burgers_non_linear.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.legend()
ax.set_xlim(-2.0, 9.0)
filename = joinpath(@__DIR__, "postprocessed_results","reactive_rp1_density.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.legend()
ax.set_xlim(-2.0, 9.0)
filename = joinpath(@__DIR__, "postprocessed_results","reactive_rp1_pressure.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.set_ylabel("\$ Y \$")
ax.grid(true, linestyle = "--")
blend = readdlm(joinpath(@__DIR__, "..", "run", "reactive_rp/reactive_rp1_blend/avg.txt"))
tvb = readdlm(joinpath(@__DIR__, "..", "run", "reactive_rp/reactive_rp1_tvb/avg.txt"))
exact_Y(x) = x <= 7.1247 ? 0.0 : 1.0
ax.plot(blend[:,1], exact_Y.(blend[:,1]), label = "Reference", lw = 2, c = "k")
plot_avg(ax, blend, ls = "--", label = "Numerical", color = "red", y_index = 5)
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results","reactive_rp1_Y.pdf")
fig
fig.savefig(filename)
run(`bash pdfbb $filename`)
...
ax.tick_params(axis="both", which="minor", labelsize=18)
ax.grid(true, linestyle = "--")
vardv_sol = readdlm(joinpath(@__DIR__, "..", "run", "vardv", "output_varadv", "sol.txt"))
x = LinRange(0.1, 1.0, 1000)
ax.plot(x, exact.(x), label = "Exact", c = "k", lw = 2, ls = "-")
plot_sol(ax, vardv_sol, 3)
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results", "var_adv.pdf")
fig.savefig(filename)
fig
run(`bash pdfbb $filename`)

# Multiion collision temperature
multiion_dir = joinpath(@__DIR__, "..", "run", "multiion_collision")
coarse_data = readdlm(joinpath(multiion_dir, "multiion_collision_coarse_data", "temperature_ion1.txt"))
fine_data = readdlm(joinpath(multiion_dir, "multiion_collision_fine_data", "temperature_ion1.txt"))
fine_data_explicit = readdlm(joinpath(multiion_dir, "multiion_collision_fine_crk33_data", "temperature_ion1.txt"))
reference_data = readdlm(joinpath(multiion_dir, "reference_data.txt"))
fig, ax = plt.subplots()
ax.set_xlabel("Time")
ax.set_ylabel("Temperature")
ax.grid(true, linestyle = "--")
ax.plot(reference_data[:,1], reference_data[:,2], linestyle = "-", label = "Reference Data", color = "black")
ax.scatter(coarse_data[2:end-1,1] .* 1000, coarse_data[2:end-1,2], label = "\$C_{\\mathrm{CFL}} = 0.08 \$ ", marker = "o", facecolors = "none", edgecolors = "red")
ax.scatter(fine_data[2:end-1,1] .* 1000, fine_data[2:end-1,2], label = "\$C_{\\mathrm{CFL}} = 0.01 \$ ", marker = "d", facecolors = "none", edgecolors = "blue")
ax.scatter(fine_data_explicit[2:end-1,1] .* 1000, fine_data_explicit[2:end-1,2], label = "\$C_{\\mathrm{CFL}} = 0.01\$, explicit", marker = "^", facecolors = "none", edgecolors = "green")
ax.set_xlim(0, maximum(reference_data[:,1]))
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results", "multiion_collision_temperature.pdf")
fig.savefig(filename)
fig
run(`bash pdfbb $filename`)

multiion_dir = joinpath(@__DIR__, "..", "run", "multiion_collision")
coarse_data = readdlm(joinpath(multiion_dir, "multiion_collision_coarse_data", "temperature_ion2.txt"))
fine_data = readdlm(joinpath(multiion_dir, "multiion_collision_fine_data", "temperature_ion2.txt"))
fine_data_explicit = readdlm(joinpath(multiion_dir, "multiion_collision_fine_crk33_data", "temperature_ion2.txt"))
reference_data = readdlm(joinpath(multiion_dir, "reference_data_t2.txt"))
fig, ax = plt.subplots()
ax.set_xlabel("Time")
ax.set_ylabel("Temperature")
ax.grid(true, linestyle = "--")
...
ax.set_xlim(0, maximum(reference_data[:,1]))
ax.legend()
filename = joinpath(@__DIR__, "postprocessed_results", "multiion_collision_temperature2.pdf")
fig.savefig(filename)
fig
run(`bash pdfbb $filename`)
