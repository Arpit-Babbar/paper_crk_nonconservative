...
function TenkaicRK.implicit_source_solve(lhs, eq, x, t, coefficient,
    ...
    # The needed 10^3 * tol is larger than usual (10 * tol), probably because of the large dt. It is fixed by reducing the cfl_safety_factor, or by refining the mesh (which also reduces dt)
    # to 0.7
    u_new = newton_solver(implicit_F, initial_guess, tol, maxiters)
    s_new = TenkaicRK.calc_source(u_new, x, t, source_terms, eq)
    @assert maximum(implicit_F(u_new))<1000 * tol implicit_F(u_new)
    return u_new, s_new
end
