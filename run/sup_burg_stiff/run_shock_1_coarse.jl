...
function TenkaicRK.implicit_source_solve(lhs, eq, x, t, coefficient,
    ...
    # The needed 10^4 * tol is larger than usual (10 * tol), probably because of the large dt. It is fixed by reducing the cfl_safety_factor, or by refining the mesh (which also reduces dt)
    # to 0.7
    @assert maximum(implicit_F(u_new))<10^4 * tol implicit_F(u_new)
