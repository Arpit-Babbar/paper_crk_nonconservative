...
function initial_condition_khi(x, t, equations::IdealGlmMhdMultiIonEquations2D)
    ...
    prim = SVector{Trixi.nvariables(equations), real(equations)}([
                                                                     B1,
                                                                     B2,
                                                                     B3,
                                                                     rho1,
...
struct ExactSolutionMultiIonKHI{Equations}
    equations::Equations
end

function (es::ExactSolutionMultiIonKHI)(x, y, t)
    initial_condition_khi(SVector(x, y), t, es.equations.trixi_equations)
end

function (es::ExactSolutionMultiIonKHI)(x, y)
    initial_condition_khi(SVector(x, y), 0.0, es.equations.trixi_equations)
end
...
exact_solution_kh = initial_value = ExactSolutionMultiIonKHI(eq)
...
problem = Problem(domain, initial_value, boundary_value, boundary_condition,
                  final_time, exact_solution_kh,
                  source_terms = source_terms_lorentz_khi
                  )
amax = 0.3
limiter_blend = setup_limiter_blend(blend_type = fo_blend(eq),
                                    ...
                                    amax = amax,
                                    pure_fv = false,
                                    positivity_blending = PositivityBlending((Eq.density,Eq.total_pressure)))
...
param = Parameters(grid_size, cfl, bounds, save_iter_interval, save_time_interval,
                   ...
                   saveto = joinpath(@__DIR__, "output_khi_$(nx)_N$(degree)_amax$amax"))
