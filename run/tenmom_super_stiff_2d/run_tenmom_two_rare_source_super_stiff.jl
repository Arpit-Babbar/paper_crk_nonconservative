...
function initial_wave(x_, y_, equations::Eq.TenMoment2D)
    x = x_
    y = y_
...
function gauss_source_x(x_, y_, t)
    x = x_
    y = y_
    a = -20 * (x^2 + y^2)
    der_factor = -x
...
function gauss_source_y(x_, y_, t)
    x = x_
    y = y_
    a = -20 * (x^2 + y^2)
    der_factor = -y
...
param = Parameters(grid_size, cfl, bounds, save_iter_interval, save_time_interval,
                   ...
                   saveto = joinpath(@__DIR__, "tmp_super_stiff_2d_nx$(nx)"))
#------------------------------------------------------------------------------
