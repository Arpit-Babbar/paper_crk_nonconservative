...
exact_brock = readdlm(joinpath(@__DIR__, "..", "..", "postprocess", "exact_brock.txt"), skipstart = 1)
...
scatter!(p_ua[2], exact_brock[:, 1] * 1.3, exact_brock[:, 2] * h0, label = "Reference",
         lw = 2)

p = plot(p_u1.subplots[2])
p_ua_ = plot(p_ua.subplots[2])
ylims!(p, (0.0, 0.013))
ylims!(p_ua_, (0.0, 0.013))